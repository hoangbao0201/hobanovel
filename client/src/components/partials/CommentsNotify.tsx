...
import { getCommentsNotifyHandle } from "@/services/comment.services"
...
export const CommentsNotify = ({ receiverId } : CommentsNotifyProps) => {
    ...
    const handleGetCommentsNotify = async () => {
        ...
        try {
            ...
            const commentsRes = await getCommentsNotifyHandle(receiverId, 1, token);
                                                    ...
                                                    comments.map(comment => {
                                                        return (
                                                            <li key={comment.commentId} className={`${ comment.isRead ? "bg-white" : "bg-[#fff2ee]" } transition-colors hover:bg-slate-100 border-b`}>
                                                                <Link className="flex px-4 py-3" href={`/truyen/${comment.slug}#comment-${comment.parentId}`}>
